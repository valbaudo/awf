...
func TestLocalDispatcherContainerSpecForImage(t *testing.T) {
	wf := &ir.Workflow{
	...
	spec := d.ContainerSpecFor(wf, "lab")
...
func TestLocalDispatcherContainerSpecForResources(t *testing.T) {
	wf := &ir.Workflow{
		Containers: map[string]ir.Container{
			"lab": {
...
func TestLocalDispatcherContainerSpecForMissingContainerDefensive(t *testing.T) {
	...
	spec := d.ContainerSpecFor(wf, "ghost")
	if spec.Name != "ghost" {
