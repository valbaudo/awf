...
func TestProjectRootSpanAndRollup(t *testing.T) {
	...
	mk := func(usd float64) engine.NodeCompletedData {
		return engine.NodeCompletedData{Outcome: "ok", Usage: &agent.MetricSet{Cost: agent.MetricCost{Total: usd, Source: agent.CostSourceReported}}}
	}
	events := []state.Event{
...
func TestProjectAgentConversationID(t *testing.T) {
	...
	events := []state.Event{
		...
		ev(t, engine.EventNodeCompleted, "triage", t0.Add(2*time.Second), engine.NodeCompletedData{Outcome: "ok", Usage: &agent.MetricSet{Cost: agent.MetricCost{Total: 0.01, Source: "reported"}}}),
	}
	spans, _ := Project(events, nil)
