...
The adoption clock must not start until a developer with an existing Claude Code subscription can follow public documentation and complete one clean-room gated run using the published v0.5.1 binary, without Docker, Ollama, a second model provider, or runtime changes.
...
- one `anthropic/claude-code` generator with `bare: false`, so it uses the operator's existing Claude Code login;
...
1. Confirm `claude --version` succeeds and Claude Code is logged in.
2. Download the v0.5.1 archive for the operator's OS/architecture and verify it against the published checksum.
3. Put the extracted `awf` binary on a temporary PATH; installation into `/usr/local/bin` is optional.
4. From the pinned fixture commit, run `awf validate examples/claude-code-gated/workflow.yaml`.
5. Run `awf run --backend native examples/claude-code-gated/workflow.yaml`.
6. Record the run id, confirm the final status is `ok`, and retrieve the generator's typed output with the documented runtime address.
...
- Claude not logged in: stop and tell the operator to authenticate; do not add API-key setup to this subscription-path fixture.
...
4. A live subscription-authenticated run succeeds on Darwin arm64 with `--backend native`.
