...
Commit(path, outputsRef, fileRefs, outcome):
   1. typed outputs + output_files already materialized in Blobs (CAS ⇒ existence is durable & verifiable)
      (for a snapshot: workspace container, its workdir snapshot is materialized here too — a CoW FS diff on docker, a full gzip-tar archive on native)
...
`with:` stays opaque to the core (each adapter validates its own schema), but the key *names* follow one convention across adapters: a concept present in more than one harness gets one shared spelling, checked at the adapter-author level, not enforced by the core. **Shared names** (same concept, same key, every adapter that has it): `allowed_tools` (tool permissioning — claude, claude-code-session, droid; droid renamed `enabled_tools` to match, F19) and `disallowed_tools` (droid-only — droid renamed `disabled_tools` to match, F19; claude and claude-code-session have no deny-list, only `allowed_tools`), `effort` (reasoning effort — claude, codex, codex-live, droid all use `effort`; droid's `reasoning_effort` was renamed to match, F12), `model`, `system_prompt`. **Harness-native names** stay when the knob is genuinely unique to one harness's design, not merely spelled differently for the same idea — e.g. codex's `sandbox` (a sandbox *policy* enum) and codex-live's `permission_policy` (a live-session permission mode) are different in kind, not aliases of each other, so neither is forced into the other's name. A rename from a harness-native spelling to the shared name is a **hard rename**: the old key is rejected with `ErrInvalidConfig{KeyUnknown: true, Reason: "renamed to <new>"}` (a per-adapter `renamedKeys` map checked before the generic unknown-key loop), never silently aliased — aliasing would let two spellings drift and multiply the surface instead of shrinking it.
...
**Claude Code adapter (first).** Runs the CLI inside the container: `claude -p <prompt> --model <m> --output-format stream-json [--max-turns N] [tool allowlist]`. Streams stream-json → `AgentEvent`s for the live tap and `agent.*` span events. Reads `total_cost_usd` + `usage` → `MetricSet{cost.source=reported, gen_ai.usage.*}`. Validates `with` keys against its own allowlist and renders them as CLI flags; passes the idempotency key into the agent env. (No MCP config is mapped: the adapter accepts no MCP key and emits no `--mcp-config` — an HTTP-transport MCP server is declared as a Compose service and reached by URL. See **agents:** `with` in awf-workflow(5).)
...
### `output_schema` fidelity tiers (F37)

Now that goose, droid, codex, and awfllm all ship, the layered typed-output story above resolves to three concrete tiers — by how tightly the harness constrains its own output to the schema before AWF ever inspects it. (The `Caps.NativeSchema` column in the capability matrix above is tier 1 vs. everything else; this is finer-grained.) A capability-matrix column enumerating all three per-adapter is deferred to the generated table (U6) — this is the prose contract it will summarize.

1. **Native schema flag** — the harness CLI itself accepts a schema argument and validates before returning: `openai/codex`'s `codex exec --output-schema <file>` (`Caps{NativeSchema: true}`). The engine's `ValidateOutputMap` still re-validates post-hoc (never trusts a harness), but for this tier it is confirmatory, not load-bearing.
2. **API-constrained** — no harness-level schema flag, but the *transport* asks a schema-aware completions API to constrain generation: `agent/awfllm`'s default (`structured_output: response_format`, OpenAI-compat strict `response_format`/`json_schema`; `ollama_format` selects Ollama's native `/api/chat` `format` field instead). Still `Caps{NativeSchema: false}` — the adapter documents the constraint as "a first-try optimization, not the contract": the schema directive (tier 3) is appended to the prompt regardless as belt-and-suspenders, and the reassembled text always goes through the same layer-2 parser as tier 3. Anthropic has no `response_format`/`json_schema` mode, so `provider: anthropic` rejects `structured_output: response_format` at validate time and falls through to tier 3.
3. **Directive+parse** — no harness or API schema constraint at all: the adapter restates the schema in-prompt (an "IMPORTANT: your FINAL message must be ONLY a single JSON object…" directive) and parses the model's free text for a JSON object as a backstop against prose or code fences. This is `block/goose` and `factory/droid` (always), and `agent/awfllm` whenever `structured_output: off` or a provider with no constrained mode. The parser is `agent.ExtractJSONObject` (`agent/schema.go`) — ONE implementation shared by all three adapters (hoisted from three byte-identical copies, F37): strip a leading fence → strict whole-text decode → else scan every `{` with `json.Decoder` (string-aware, so braces/quotes inside string values can't fool it), right-biased (the LAST object that decodes wins, so a reasoning preamble's decoy object loses to the final answer). A parse failure surfaces as `*agent.ErrUnparseableOutput` (`retryable_failure`) — never a fabricated repair via a json-repair dependency.

The schema **directive text** (as opposed to the parser) is deliberately NOT shared: awfllm, goose, and droid each word it slightly differently (goose's includes a worked example; awfllm silently drops a marshal failure and leaves the prompt unchanged where goose/droid propagate the error), so a single `AppendSchemaDirective` helper would either force a parameter per call site (little consolidation) or homogenize three agents' prompts under refactor cover (a behavior change, not a hoist) — YAGNI'd for now; revisit only if a fourth adapter needs the *same* wording verbatim.
