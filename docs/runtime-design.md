...
### IR (`ir`)
A `Workflow{ID, Version, Input *JSONSchema, Imports map[string]Import, Assets map[string]Asset, Containers map[string]Container, Graph []Node, OutputSchema *JSONSchema, Outputs map[string]Template, OutputFiles map[string]ArtifactRef, Digest string}`. `Digest` is a **self-describing** content hash (`awf-d1:sha256:…`) computed at load: the whole resolved IR marshaled to JSON (explicit `json` tags on every field) then RFC-8785 JCS — so it is independent of Go field/map order — folding in imported workflow definitions, referenced compose-file hashes, and asset bytes (spec §8 / Phase-1 design spec). `Node` is a sum type:
...
Pure function `Path(parent, node, siblingIndex, branch) string` implementing spec §4: step nodes by `id`; control nodes by `keyword[index].branch` (`try[0].catch`, `if[2].else`, `loop[0].body.iter-3`, `gate[0].attempt-2.generate`, `parallel[1]`). Used as the journal key and OTel `awf.node.path`.
...
Event types: `run.started{input_ref, workflow_digest, backend, runtimes, assets}`, `run.finished`, `run.paused`, `run.skipped{reason}`, `node.started`, `node.completed{outputs_ref, files_ref, snapshot_ref?, outcome}`, `node.failed{outcome, error}`, `call.started{input_ref,runtimes}`, `branch.taken{which}`, `loop.iter{n}`, `map.item{n, image_digest?, reason?}`, `retry.attempt{n, error, outcome}`, `signal.received{signal, payload_ref}`, `io.chunk{stream, ref}`, `agent.event{ref}`, `agent.tool.call/result`. Control-flow decisions that affect the resume path (`call.started`, `branch.taken`, `loop.iter`, `map.item`, `signal.received`, `run.skipped`) are events so the re-walk is deterministic.
...
### Run start
Load imports + validate → compute `Digest` → resolve each `uses:` to a concrete adapter **identity + version** → select a concrete backend → snapshot top-level assets into `Blobs` → open log → append `run.started{input_ref, workflow_digest, backend, runtimes:[{ref,version}], assets}` (input validated against `Workflow.Input`). `--backend auto` selection belongs in CLI run before backend construction and before `run.started`: auto records `native` unless Docker-only workflow features are present, in which case it records `docker`. Docker-only features include static image-backed containers, compose-backed containers, workspace snapshots, runtime compose, and runtime map images. When auto records native, `awf run` prints the native-resume warning and guidance to use `--backend docker` for resumable runs. Explicit `--backend native` keeps the existing behavior and does not print that auto-selection warning. On resume, the on-disk resolved definition's digest (including current imported workflow files and asset bytes) **and** every resolved runtime version **MUST** equal the logged values, else hard error (spec §8); resume uses the backend kind recorded in `run.started`, never re-runs auto-selection, and a recorded native backend fails with the existing native-resume limitation plus the same `--backend docker` guidance.
...
- **`call`**: resolve `input` against the caller scope, materialize it in Blobs, resolve the imported workflow's runtimes, then append and fsync `call.started{input_ref,runtimes}` before the child workflow graph executes. Fold materializes `RunState.CallStarted[path]` so resume reuses the exact call input and checks call-level runtime drift before replaying the child frontier. Child workflow nodes execute under `<call-id>.workflow.<child-path>`. The call node commits at `<call-id>` with typed outputs and file aliases.
...
type Backend interface {
    Capabilities() Caps                 // {Snapshot: [none, fs-cow], RuntimeImage: bool}  (RuntimeImage = P6a)
...
| `awf run <file> [--input <json>] [--run-id <id>] [--state-dir <dir>] [--backend auto\|fake\|docker\|native] [--agent-env <csv>]` | Execute; print run.id; live tap to console. Default `auto` records native unless Docker-only workflow features select docker; auto-native prints the native-resume warning and `--backend docker` guidance. |
| `awf resume <run.id>` | Verify digest + runtime versions, fold log including call-start records, use the recorded backend, recreate containers from recipe, continue in a new epoch; reject recorded native runs with the existing native-resume limitation and `--backend docker` guidance. |
...
| `awf inspect <run.id> [--fold <statuses>] [--depth <n>] [--output text\|json]` | Offline: render the addressing tree as a fold-by-status text tree. |
...
- **Conformance suite (the bar):** drives the §8 guarantees against the **fake backend**:
  1. *Pinning* — resume against a mutated definition **or** a drifted runtime version is rejected.
