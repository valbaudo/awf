...
func (r *Runner) cliResume(args []string, stdout, stderr io.Writer) int {
	...
	if kind == engine.BackendNative {
		fprintf(stderr, "awf resume: native backend — committed work is replayed and snapshot: workspace workdirs are restored from a full workdir archive, but the host base environment is not pinned; shell-step tooling runs against the current host.\n")
	}
