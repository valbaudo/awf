...
// (slice 2.4 fills AWF_OUTPUT and AWF_IDEMPOTENCY_KEY); the Phase 2 fake
// accepts Env but does not use it (its scripted result table is keyed on Run
// alone — slice 2.4 verifies dispatcher env injection by inspecting what it
// passes to Backend.Exec, not by what the fake does with the env it receives).
type Cmd struct {
	Run string
