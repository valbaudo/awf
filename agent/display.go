...
//
// Text by Class: Assistant/Final → the full text; AssistantDelta → one
// incremental chunk of assistant text (the renderer writes it without a trailing
// newline so a stream of deltas renders character-by-character; the durable
// record is the per-event Payload as usual); Reasoning → the reasoning text
