...
// same UUID; different NodePath → different UUID. It is the value passed to
// claude's --session-id (fresh turn) / --resume (restored turn) flags.
//
// Algorithm: sha256("runID|epoch|nodePath"), first 16 bytes, formatted as a
// standard 8-4-4-4-12 UUID string (the value is opaque to Claude Code, which
// only needs a stable unique string per session).
